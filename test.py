...
class Player(tk.Frame):
	...
	def __init__(self, master=None):
		super().__init__(master) # giving the function property to the class
		self.master = master
		self.pack()
		mixer.init()
	...
	def create_frames(self):
		self.track = tk.LabelFrame(self, text='Song Track',
					...
					bg="mistyrose",fg="dodgerblue",bd=5,relief=tk.GROOVE)
	...
	# Creating track widgets
	def track_widgets(self):
		self.canvas = tk.Label(self.track, image=img_render)
		self.canvas.place(x=0, y=0)
		self.canvas.configure(width=400, height=240)
		self.canvas.grid(row=0,column=0)

		self.songtrack = tk.Label(self.track, font=("times new roman",16,"bold"),
						bg="mistyrose",fg="dodgerblue")
		self.songtrack['text'] = 'Soul Sound - MP3 Player'
		self.songtrack.config(width=30, height=1)
		self.songtrack.grid(row=1,column=0,padx=10)

	# Creating controller widgets
	def control_widgets(self):
		self.loadSongs = tk.Button(self.controls, bg='mistyrose', fg='dodgerblue', font=10)
	...
	# Gives a specific identity to an element in the list
	...
	def switch(self):
		if self.btnState:
			self.btn.config(image=offImg_render, bg="#CECCBE", activebackground="#CECCBE")
			root.config(bg="#CECCBE")
			self.txt.config(text='Dark Mode: OFF', bg='#CECCBE')
			self.btnState = False
		else:
			self.btn.config(image=onImg_render, bg="#3d3d3d", activebackground="#3d3d3d")
			root.config(bg="#3d3d3d")
			self.txt.config(text='Dark Mode: ON', bg='#3d3d3d')
			self.btnState = True
	...
	def Settings(self):
		...
		self.btn = tk.Button(win, image=offImg_render, bg="#CECCBE", activebackground="#CECCBE", command=self.switch)
		self.btn.place(x=100, y=100)
		self.txt = tk.Label(win, text="Dark Mode", bg="#CECCBE", fg='green')
...
root.wm_title('Soul Sound - MP3 Player')
root.iconbitmap(r'images/logo1.ico')
root.config(bg="#CECCBE")

# Defining and inserting images
...
offImg = Image.open('images/toggle.png')
offImg_render = ImageTk.PhotoImage(offImg)
onImg = Image.open('images/toggle1.png')
onImg_render = ImageTk.PhotoImage(onImg)
...
SubMenu.add_command(label='Load Songs', command=Player.retrieve_songs)
SubMenu.add_command(label='Exit', command=root.destroy)

SubMenu = tk.Menu(Menubar, tearoff=0)
Menubar.add_cascade(label="Help", menu=SubMenu)
SubMenu.add_command(label='Settings', command=Player.Settings)
SubMenu.add_command(label='About Us')

app = Player(master=root)
app.mainloop() # Closing the GUI
